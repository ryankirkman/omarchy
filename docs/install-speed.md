...
The next selected trial adds [firewall scheduling](../test/benchmarks/install-speed/firewall-overlap/README.md) and [logging optimized only for serial system setup](../test/benchmarks/install-speed/logging-bind/README.md). Every original firewall command must finish at the start of the user branch, before user, SSH and indexing work. A temporary private read-only mount exposes the faster logger during the large system-setup call, preserving installed package files. Applying that mount to all four setup calls regressed in the complete TCG component screen and was rejected. The narrower version improved all three fresh component pairs, with medians 15.316 to 12.077 seconds (1.26826×); its one-time controller import is outside that component timer but remains inside native full-install timing. All 56 selected host component checks, actual private-mount success/failure checks and final payload composition passed. This follow-up has no native install-speed result yet.
