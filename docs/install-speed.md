...
The comparison tool consumes run directories containing `manifest.json`, `install-timing.json`, `validation.json`, and `package-manifest.txt`. It rejects unbooted, incomplete, reused, different-hardware and different-package samples, or failed package-file verification. For three or more fresh samples per treatment it additionally reports whether even the fastest baseline divided by the slowest candidate exceeds two. It keeps boot-to-SSH timings separate from the guest installer interval.
