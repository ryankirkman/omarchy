...
After the measured SSH observation, collection derives `install-log-events.json` from the installer log it already reads, without another guest command. Only complete `run_logged` Starting, Completed or Failed records for safe `/usr/share/omarchy/install/*.sh` paths survive, with timestamp text and an exit code for failures; raw messages, commands and environment output are excluded. At most 512 recognized events are retained, with explicit truncation if more exist. These are advisory guest-local wall-clock stamps at one-second resolution, with no assumed timezone or monotonic duration. Missing logs and absent recognized events remain advisory and do not change sample acceptance. Repeated-run sealing retains this small derived file while excluding the raw installer log. Run `bash test/shell.d/iso-vm-install-log-events-test.sh` for parser, collection and retention checks.

The focused checks passed with the actual collection and sealing functions, including unchanged guest-command counts and timing fields, missing-log tolerance, package-validation failure, raw-log exclusion and seal tamper rejection. A [read-only check of two preexisting local TCG logs](install-speed/results/install-log-events/local-tcg-parser-check.json) recognized 126 events covering 63 script paths in each, without truncation; it retains source/parser hashes and derived fields, not the raw logs. This validates parsing on real log data and is not a new installation or native timing result.
