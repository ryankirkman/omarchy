...
def main():
  ...
  parser.add_argument('--logging-scope', choices=('serial-system-finalizer-only', 'all-target-setup-commands'),
    default='serial-system-finalizer-only')
  ...
  report = {'schema_version': 1, 'scope': 'Actual private file bind and arch-chroot on an owned minimal /tmp fixture; no installed disk, package changes or installation-speed claim',
    'logging_scope': args.logging_scope,
...
'''
    ...
    if args.timing_pairs:
      ...
      for pair in range(args.timing_pairs):
        ...
        for label in order:
          ...
          for number, leaves in enumerate((49, 1, 12, 1)):
            ...
            body += ('logger_body=$(declare -f run_logged)\n'
              'if [[ $logger_body == *\'printf -v timestamp\'* ]]; then '
              'printf "LOGGING_COMPONENT_HELPER=optimized\\n"; else '
              'printf "LOGGING_COMPONENT_HELPER=original\\n"; fi\n')
            if number == 0:
            ...
            use_guard = label == 'candidate' and (number == 0 or args.logging_scope == 'all-target-setup-commands')
            ...
            helper_kind = 'optimized' if use_guard else 'original'
            if text.count('LOGGING_COMPONENT_HELPER=') != 1 or f'LOGGING_COMPONENT_HELPER={helper_kind}\n' not in text:
              raise RuntimeError('Timed component loaded a logger outside its selected scope')
            if module.mount_record(logger) is not None or module.file_record(logger, module.ORIGINAL_LOGGER_SHA256) != original:
              raise RuntimeError('Timed invocation changed the original package file or parent mount')
            phase_rows.append({'leaves': leaves, 'seconds': elapsed,
              'execution_path': 'private-logging-bind' if use_guard else 'original-private-chroot',
              'observed_helper': helper_kind,
              'original_package_file_and_parent_mount_unchanged': True})
          expected_guard_calls = (1 if args.logging_scope == 'serial-system-finalizer-only' else 4) if label == 'candidate' else 0
          if sum(row['execution_path'] == 'private-logging-bind' for row in phase_rows) != expected_guard_calls:
            raise RuntimeError('Timed component logging scope or private lifecycle count differs')
      ...
      report['component_timing'] = {'scope': 'TCG guest only; real Bash/date and four private chroot lifecycles, 63 trivial leaves; no native or full-install inference',
        'logging_scope': args.logging_scope,
        'candidate_guard_calls_per_sample': 1 if args.logging_scope == 'serial-system-finalizer-only' else 4,
