...
def main():
  ...
  with tempfile.TemporaryDirectory(prefix='omarchy-logging-bind-guest-', dir='/tmp') as temporary:
    ...
    (target / 'tmp/leaf.sh').write_text("printf 'LEAF_OUTPUT\\n'\n")
    (target / 'tmp/leaf.sh').chmod(0o644)
...
run_logged /tmp/leaf.sh
bash -c 'source /usr/share/omarchy/install/helpers/logging.sh; run_logged /tmp/leaf.sh'
stop_install_log
printf 'GUARD_GUEST_UID=%s\\n' "$EUID"
'''
      handshake = label == 'root-success'
      if handshake:
        os.mkfifo(target / 'tmp/release', 0o600)
        script += "printf ready >/tmp/ready\nread -r release </tmp/release\n[[ $release == continue ]]\n"
      ...
      try:
        ...
        if handshake:
          ...
          while not (target / 'tmp/ready').exists() and child.poll() is None and time.monotonic() < deadline:
            time.sleep(0.05)
          if not (target / 'tmp/ready').exists():
            raise RuntimeError('Real mounted child did not reach the isolation handshake')
          ...
          with (target / 'tmp/release').open('w') as release:
      ...
      if len(re.findall(r'\[\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}\] (?:Starting|Completed): /tmp/leaf.sh', stdout)) != 4:
      ...
      for pair in range(args.timing_pairs):
        ...
        for label in order:
          ...
          for number, leaves in enumerate((49, 1, 12, 1)):
            ...
            body += f'for ((i=0;i<{leaves};i++)); do run_logged /tmp/leaf.sh; done\n'
            if number == 0:
            ...
            if status != 0 or text.count('LEAF_OUTPUT\n') != leaves or text.count('] Completed: /tmp/leaf.sh\n') != leaves:
