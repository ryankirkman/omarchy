...
if name == "installer":
  ...
  if settings["overlap"]:
    ...
    # The first query is the explicit redraw. Hold through two more loop
    # queries so at least one whole later poll sees finalizing unchanged.
    wait_for(lambda: progress_frames(after_effect=True) >= 3,
             "installer waiting for three finalizing progress frames after effect")
  else:
...
def exercise(directory, dashboard, name, **changes):
  ...
  env = {key: value for key, value in os.environ.items()
         if not key.startswith(("OMARCHY_", "BASH_FUNC_"))
  ...
  try:
    ...
    if settings["overlap"]:
      ...
      require(ui.count(b"Finalizing Omarchy") == 1, f"{name}: early finish frame repeated")
      ...
      if not settings["child_rc"]:
        ...
        require(len(frames) >= 3, f"{name}: did not exercise repeated finalizing polls")
    else:
