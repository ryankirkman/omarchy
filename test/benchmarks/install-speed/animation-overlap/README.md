...
The host contract rebases only the executed test copy's serial-marker destination to a fresh nonexistent path inside its owned temporary directory. It checks the unchanged runtime dashboard hash, requires one exact marker block, and asserts before every full-dashboard execution that no host serial path remains. Runtime serial markers and their log copies are unchanged. This permits hosts with a real `/dev/ttyS0`; the test never inspects or writes that device. Native attempt `34004881555` stopped at the earlier host-device guard before paired installs, which motivated this test-isolation fix and supplies no paired performance result.
