...
def capture(directory, hostname, qmp, *, prefix="", timeout=CONSOLE_SECONDS):
  ...
  connection = None
  try:
    serial_devices = [row for row in qmp("query-chardev") if row.get("label") == "serial0"]
    expected = "file:" + str(serial)
    if (len(serial_devices) != 1 or serial_devices[0].get("filename") != expected
        or serial_devices[0].get("frontend-open") is not True or serial.is_symlink()):
