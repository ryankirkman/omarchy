...
class Supervisor:
  ...
  def capture_timeout_diagnostics(self, reason, prefix=""):
    ...
    capture("live-console", lambda: self.capture_live_console(prefix))

  def capture_live_console(self, prefix):
    # Imported only on a failed run; no extra guest work enters install timing.
    path = Path(__file__).with_name("postfailure-console.py")
